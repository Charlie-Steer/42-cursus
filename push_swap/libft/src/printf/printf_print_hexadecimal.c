...
/*   Updated: 2024/09/25 14:10:27 by cargonz2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
				...
				int arg_len)
{
	...
	if (cs.has_point && cs.point_width == 0 && n_str[0] == '0')
		...
		right_pad_comp = pf_max(arg_len, cs.point_width);
	if (cs.has_alternate)
		right_pad_comp += 2;
...
int	pf_print_hexadecimal(unsigned int n, t_conv_spec cs)
{
	...
	n_str = pf_itoa_base(n, pf_get_hex_base(cs.conv_specifier));
	...
	if (!print_str)
		return (-1);
	print_str += pf_hexa_add_prefix(print_str, cs, n_str, arg_len);
